import streamlit as stimport pandas as pdimport numpy as npimport torchimport pickleimport osimport timefrom transformers import AutoModelForSequenceClassification, AutoTokenizerfrom sklearn.metrics import accuracy_score, f1_score, confusion_matriximport matplotlib.pyplot as pltimport seaborn as sns
...
MODEL_TFIDF_SENT          = "./models/tfidf/sentiment/baseline_sentiment_model.pkl"LABEL_ENCODER_TFIDF_SENT  = "./models/tfidf/sentiment/baseline_sentiment_label_encoder.pkl"

TF-IDF Topic

MODEL_TFIDF_TOPIC         = "./models/tfidf/topic/baseline_topic_model.pkl"LABEL_ENCODER_TFIDF_TOPIC = "./models/tfidf/topic/baseline_topic_label_encoder.pkl"

DATASET_EXCEL = "./dataset/train.xlsx"VALID_PATH    = "./dataset/validation.xlsx"

TOPIC_MAP_VI = {"curriculum": "Chương trình đào tạo 📚","facility":   "Cơ sở vật chất 🏫","lecturer":   "Giảng viên 👨‍🏫","other":      "Khác / Tổng quan 📝",}

def topic_vi(label: str) -> str:return TOPIC_MAP_VI.get(str(label).strip().lower(), label)

def _require_file(path: str, label: str):if not os.path.exists(path):st.error(f"❌ Không tìm thấy file bắt buộc {label}:\n{path}")st.stop()
...
@st.cache_resourcedef load_all_models():device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
try:
    ...
    st.error(f"❌ Lỗi load PhoBERT Sentiment: {e}"); st.stop()
with open(LABEL_ENCODER_PHOBERT, "rb") as f:
    phobert_sent_le = pickle.load(f)

# ── PhoBERT Topic (optional) ──────────────────────────────────────────────
...
if os.path.exists(os.path.join(MODEL_PHOBERT_TOPIC, "model.safetensors")):
    try:
        ...
        st.warning(f"⚠️ PhoBERT Topic chưa load được: {e}")
        phobert_topic_m = phobert_topic_t = phobert_topic_le = None
...
(tfidf_sent_m,  tfidf_sent_le,tfidf_topic_m, tfidf_topic_le,phobert_sent_m, phobert_sent_t, phobert_sent_le,phobert_topic_m, phobert_topic_t, phobert_topic_le,DEVICE) = load_all_models()
...
@torch.no_grad()def phobert_predict_batch(texts, model, tokenizer, le, batch_size=32):all_labels = []for i in range(0, len(texts), batch_size):chunk  = texts[i: i + batch_size]inputs = tokenizer(chunk, return_tensors="pt", truncation=True,padding=True, max_length=128)inputs = {k: v.to(DEVICE) for k, v in inputs.items()}logits = model(**inputs).logitsids    = torch.argmax(logits, dim=-1).cpu().numpy()all_labels.extend(le.inverse_transform(ids))return all_labels

def tfidf_predict(texts, model, le):codes = model.predict(texts)return le.inverse_transform(codes)
...
def standardize_sentiment(label_list):out = []for l in label_list:s = str(l).strip().lower()if any(x in s for x in ["tiêu cực", "neg", "negative"]) or s in ["0", "0.0"]:out.append("0")elif any(x in s for x in ["trung lập", "neu", "neutral"]) or s in ["1", "1.0"]:out.append("1")elif any(x in s for x in ["tích cực", "pos", "positive"]) or s in ["2", "2.0"]:out.append("2")else:out.append(s)return out

def label_to_display(label_text):t = str(label_text).upper()if any(w in t for w in ["POS", "TÍCH CỰC", "2", "POSITIVE"]):st.success("🎯 TÍCH CỰC 😍")elif any(w in t for w in ["NEG", "TIÊU CỰC", "0", "NEGATIVE"]):st.error("🎯 TIÊU CỰC 😡")else:st.warning("🎯 TRUNG LẬP 😐")
...
st.sidebar.title("🎮 Hệ Thống Điều Khiển")page = st.sidebar.radio("Danh mục trang:", ["🏠 Giới thiệu dự án & Dataset","⚡ Trình dự đoán song song","📊 Chỉ số thực nghiệm & Ma trận nhầm lẫn",])
...
if page == "🏠 Giới thiệu dự án & Dataset":st.title("🔮 VietText Analyzer — NLP Research Dashboard")st.markdown("### Phân tích Sắc thái và Chủ đề Ý kiến Sinh viên bằng Machine Learning & Deep Learning")st.divider()
...
*PhoBERT Topic đang được train trên Kaggle, sẽ cập nhật sau.
""")
...
elif page == "⚡ Trình dự đoán song song":st.title("⚡ Real-time Multi-Model Inference Dashboard")st.markdown("Nhập câu đánh giá — hệ thống phân tích sắc thái và chủ đề song song.")

user_input = st.text_area("✍️ Nhập nội dung ý kiến cần phân tích:",
...
if st.button("Kích hoạt phân tích 🚀", type="primary"):
    if not user_input.strip():
        ...
        with col2:
            ...
            try:
                label = phobert_predict_batch([processed], phobert_sent_m,
                                              phobert_sent_t, phobert_sent_le)[0]
                label_to_display(label)
            except Exception as e:
        ...
        with col_t2:
            ...
            if has_phobert_topic:
                try:
                    label = phobert_predict_batch([processed], phobert_topic_m,
                                                  phobert_topic_t, phobert_topic_le)[0]
                    st.info(f"📌 {topic_vi(label)}")
                except Exception as e:
                    st.error(f"Lỗi PhoBERT Topic: {e}")
            else:
                st.info("⏳ Đang train trên Kaggle — sẽ cập nhật sau.")
...
elif page == "📊 Chỉ số thực nghiệm & Ma trận nhầm lẫn":st.title("📊 Model Performance — Live Evaluation")
...
if st.button("Bắt đầu đánh giá toàn bộ tập validation 🚀", type="primary"):
    ...
    try:
        ...
        st.error(f"❌ Lỗi TF-IDF: {e}"); st.stop()
    ...
    for i in range(0, len(cleaned), PHOBERT_BATCH):
        ...
        try:
            y_pred_phobert_sent_raw.extend(
                phobert_predict_batch(cleaned[i:i+PHOBERT_BATCH],
                                     phobert_sent_m, phobert_sent_t, phobert_sent_le)
            )
        except Exception as e:
            st.error(f"❌ Lỗi PhoBERT Sentiment batch {b}: {e}"); st.stop()
    ...
    if phobert_topic_m is not None:
        for i in range(0, len(cleaned), PHOBERT_BATCH):
            ...
            try:
                ...
                st.error(f"❌ Lỗi PhoBERT Topic batch {b}: {e}"); st.stop()
    ...
    with sc1:
        st.pyplot(plot_cm(y_true_sent, y_pred_tfidf_sent,
                          SENT_LABELS, SENT_TICKS, "TF-IDF Sentiment", cmap="Blues"))
    with sc2:
        st.pyplot(plot_cm(y_true_sent, y_pred_phobert_sent,
                          SENT_LABELS, SENT_TICKS, "PhoBERT Sentiment", cmap="Blues"))
    ...
    topic_results = [{"Mô hình": "TF-IDF + ML (Topic)",
    ...
    if y_pred_phobert_topic:
        ...
        st.caption("⏳ PhoBERT Topic chưa sẵn sàng — chỉ hiển thị TF-IDF.")
    ...
    if y_pred_phobert_topic:
        ...
        with tc1:
            st.pyplot(plot_cm(y_true_topic, y_pred_tfidf_topic,
                              TOPIC_LABELS, TOPIC_TICKS, "TF-IDF Topic", cmap="Greens"))
        with tc2:
            st.pyplot(plot_cm(y_true_topic, y_pred_phobert_topic,
                              TOPIC_LABELS, TOPIC_TICKS, "PhoBERT Topic", cmap="Greens"))
    else:
        st.pyplot(plot_cm(y_true_topic, y_pred_tfidf_topic,
                          TOPIC_LABELS, TOPIC_TICKS, "TF-IDF Topic", cmap="Greens"))
